	...
	input.db.DailyReport.findAndCountAll({
		...
		if (err) {
			...
			for(var i= 0,maxi=rows.length; i<maxi; i++){
				...
				dailyInboundThroughputArray.push(daily_sum_inbound_throughput_total);
				sum += daily_sum_inbound_throughput_total;
			}

			var max = _.max(dailyInboundThroughputArray);
			var min = _.min(dailyInboundThroughputArray);
